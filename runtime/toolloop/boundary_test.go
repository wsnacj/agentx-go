...
func TestExactExperimentalSurface(t *testing.T) {
	...
	want := []string{
		...
		"RoundPhase",
		"RoundPhaseAct",
		"RoundPhaseActionCompleted",
		"RoundPhaseBeforeAction",
		"RoundPhaseCoordinator",
		"RoundPhaseExecutor",
		"RoundPhaseHostStopped",
		"RoundPhaseInput",
		"RoundPhaseNoAction",
		"RoundPhaseObserve",
		"RoundPhaseRequest",
		"RoundPhaseResult",
		"RoundPhaseResultKind",
		"RoundRequestResult",
		"RoundState",
		"RunObservation",
		"Runtime",
		...
		"StepResult",
		"Stepper",
		"TerminationFailureFuse",
		"TerminationHostPolicy",
		"TerminationKind",
		"TerminationLoop",
		"TerminationSignal",
	}
...
func TestOwnerImportDirection(t *testing.T) {
	for _, path := range productionGoFiles(t) {
		...
		for _, spec := range parsed.Imports {
			...
			if strings.HasPrefix(imported, "github.com/") ||
				strings.HasPrefix(imported, "hs/") ||
				strings.HasPrefix(imported, "scene/") {
				t.Errorf("%s imports non-standard owner %q", path, imported)
			}
		}
	}
}
