	...
	for _, path := range productionGoFiles(t, ".") {
		for _, imported := range importsOf(t, path) {
			if strings.HasPrefix(imported, "github.com/wsnacj/agentx-go/") ||
				...
				t.Errorf("%s imports non-stdlib owner %q", path, imported)
			}
		}
	}
	for _, path := range productionGoFiles(t, "..") {
		for _, imported := range importsOf(t, path) {
			if imported == "github.com/wsnacj/agentx-go/runtime/workflow/schema" {
				t.Errorf("parent workflow owner reverse-imports schema in %s", path)
			}
		}
	}
}
