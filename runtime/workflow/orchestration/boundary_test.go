...
var allowedAgentXImports = map[string]bool{
	...
	"github.com/wsnacj/agentx-go/runtime/workflow/bindingstate": true,
	"github.com/wsnacj/agentx-go/runtime/workflow/journal":      true,
	"github.com/wsnacj/agentx-go/runtime/workflow/nodeexec":     true,
	"github.com/wsnacj/agentx-go/runtime/workflow/transition":   true,
}
...
func TestOwnerImportDirection(t *testing.T) {
	...
	for _, path := range productionGoFiles(t, "..") {
		for _, imported := range importsOf(t, path) {
			if imported == "github.com/wsnacj/agentx-go/runtime/workflow/orchestration" {
				t.Errorf("parent workflow owner reverse-imports orchestration in %s", path)
			}
		}
	}
}
