...
const (
	nodeexecImportPath = "github.com/wsnacj/agentx-go/runtime/workflow/nodeexec"
	workflowImportPath = "github.com/wsnacj/agentx-go/runtime/workflow"
)

func TestOwnerImportDirection(t *testing.T) {
	for _, path := range productionGoFiles(t, ".") {
		for _, imported := range importsOf(t, path) {
			if imported == workflowImportPath {
				continue
			}
