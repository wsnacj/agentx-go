...
func TestLoadConfigUsesExplicitValuesWithoutReadingProviderDefaults(t *testing.T) {
	...
	if cfg.BaseURL != values[envBaseURL] || cfg.APIKey != values[envAPIKey] || cfg.Model != values[envModel] || cfg.Prompt != "hello" || cfg.Expect != "world" || cfg.Timeout != 12*time.Second {
...
func TestRunComposesCanonicalRuntimeAndProvider(t *testing.T) {
	...
	if value.Status != "completed" || !value.Matched || value.Reply != defaultMarker || value.SessionID != "live-provider-smoke" {
