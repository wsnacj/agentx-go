...
本页只评估当前三个 module 中实际存在的23个 production package。它不是历史
...
| `runtime/workflow` | Experimental extension | [API](../../runtime/workflow/API.md) | Workflow Spec 合同 |
...
1. 三个 module 当前 production package 与矩阵一一对应；
2. 每个 package 都有非空中文 Reference；
3. 五个 Developer Preview candidate 的 `go doc -all` 签名未漂移。
