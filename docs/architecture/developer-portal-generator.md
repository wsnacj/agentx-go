...
状态：M6B accepted implementation decision，仍属于private Developer Preview。

## 结论

Core中文Developer Portal使用VitePress 1.6.4稳定线。版本通过npm registry的
...
- Node依赖只服务Portal构建，不进入四个Go module或普通consumer依赖闭包。
...
公共托管、域名、analytics、交互API控制台和发布授权不属于M6B。
