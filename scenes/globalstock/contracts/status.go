...
// FailureCode describes source-neutral failure categories for global-stock tools.
...
// TradingSession describes the market-time status of a stock data payload.
...
// FreshnessMode describes whether data is realtime, intraday, EOD, historical, delayed, or cached.
...
const (
	...
	FreshnessModeDelayed    FreshnessMode = "delayed"
...
// SourceEvidence captures source-level provenance common to all global-stock tools.
...
// ProviderAttempt records one attempted provider in a fallback chain.
...
func repairHint(failure FailureCode, missingFields []string, reviewFields []string) string {
	...
	case FailureCodeIdentityNotFound:
		return "resolve_verified_global_stock_subject"
	...
	case FailureCodeStaleData:
		return "refresh_latest_market_source"
	case FailureCodeUnsupported:
		return "use_supported_global_stock_task_or_adapter"
	}
